...
export default function SearchBar({ search, setSearch, type, setType }: Props) {
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-4 w-full max-w-4xl items-end">
            <div className="flex-[2]">
                <FloatingLabelInput label="Name" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <div className="flex-[1.2]">
                <FloatingLabelInput label="Birthday" type="date" value={birthday} onChange={(e) => setBirthday(e.target.value)} />
            </div>
            {/* Styled dropdown with floating label mimic */}
            ...
            <button className="bg-red-900 text-white p-3 rounded-lg shadow hover:bg-red-800">
        ...
        <div className="relative flex items-center gap-4">
          <div ref={notifRef} className="relative">
            <button onClick={() => setShowNotifications(!showNotifications)}>
              <BellIcon className="h-6 w-6 text-gray-500 hover:text-red-800" />
            </button>
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-48 bg-white shadow-md border rounded-md z-50 p-3 text-sm text-gray-700">
                No notifications
              </div>
            )}
          </div>
          ...
          <div ref={settingsRef} className="relative">
            <button onClick={() => setShowSettings(!showSettings)}>
              <Cog6ToothIcon className="h-6 w-6 text-gray-500 hover:text-red-800" />
            </button>
            {showSettings && (
              <div className="absolute right-0 mt-2 w-48 bg-white shadow-md border rounded-md z-50 text-sm">
